...
final class Keccak
{
     ...
     */
    private static function unpackInts(string $format, string $data): array
    {
        $result = unpack($format, $data);
        if (false === $result) {
            throw new RuntimeException('unpack() failed for format ' . $format);
        }
    ...
    private static function keccak64(string $in_raw, int $capacity, int $outputlength, int $suffix, bool $raw_output): string
    {
        ...
        for ($in_t = 0; $inlen >= $rsiz; $inlen -= $rsiz, $in_t += $rsiz) {
            for ($i = 0; $i < $rsizw; $i++) {
                $t = self::unpackInts('V*', mb_substr($in_raw, (int) ($i * 8 + $in_t), 8, self::ENCODING));
        ...
        $temp = mb_substr($in_raw, (int) $in_t, $inlen, self::ENCODING);
        $temp = str_pad($temp, $rsiz, "\x0", STR_PAD_RIGHT);
        $temp = substr_replace($temp, chr($suffix & 0xFF), $inlen, 1);
        $temp = substr_replace($temp, chr((ord($temp[$rsiz - 1]) | 0x80) & 0xFF), $rsiz - 1, 1);

        for ($i = 0; $i < $rsizw; $i++) {
            $t = self::unpackInts('V*', mb_substr($temp, $i * 8, 8, self::ENCODING));
        ...
        $r = mb_substr($out, 0, intdiv($outputlength, 8), self::ENCODING);
    ...
    private static function keccak32(string $in_raw, int $capacity, int $outputlength, int $suffix, bool $raw_output): string
    {
        ...
        $temp = str_pad($temp, $rsiz, "\x0", STR_PAD_RIGHT);
        $temp = substr_replace($temp, chr($suffix & 0xFF), $inlen, 1);
        $temp = substr_replace($temp, chr(((int) $temp[$rsiz - 1] | 0x80) & 0xFF), $rsiz - 1, 1);
